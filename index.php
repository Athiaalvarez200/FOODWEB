...
<body>
    ...
    <main class="user_main">
        ...
        <div class="food_cards flex gap wrap justify-center">
            ...
            $apiData = api_get("products.php?active=1");
            $foods = [];
            if ($apiData && isset($apiData['success']) && $apiData['success'] && isset($apiData['products'])) {
                foreach ($apiData['products'] as $foodItem) {
                    if (isset($foodItem['special']) && $foodItem['special'] == 1) $foods[] = $foodItem;
                }
            }
            if (count($foods) > 0) {
    ...
    ₱<?php echo number_format($data['price'], 2); ?>
            ...
            <?php
                ...
                echo "<h3>API is offline. No food data can be displayed.</h3>";
            }
            ?>
        </div>
        <!-- card end -->
