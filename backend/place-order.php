...
if (isset($data['name']) && isset($data['phone']) && isset($data['address'])) {
    ...
    function placeOrder($f_id, $q, $track_id, $uid, $note, $conn, $o_c_id, $delivery_date, $delivery_time, $pm)
    {
        ...
        // Save order through the System Integration REST API.
        $apiOrder = api_post('orders.php', [
            'c_id' => $uid,
            'track_id' => $track_id,
            'o_c_id' => $o_c_id,
            'qty' => $q,
        ...
        if (!$apiOrder || !isset($apiOrder['success']) || !$apiOrder['success']) {
            die("Could not place order because REST API is offline");
        }
        $res = true;
        $order_id = intval($apiOrder['id']);
        ...
        if ($delivery_date == "0000-00-00" && $delivery_time == "00:00:00") {
            ...
            mysqli_query($conn, $sql_future_orders) or die("Could not insert into future orders");
        }
